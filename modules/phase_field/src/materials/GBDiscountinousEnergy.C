...
GBDiscountinousEnergy::GBDiscountinousEnergy(const std::string & name,
    ...
    _Q(getParam<Real>("Q")),
    _GBenergy(0),
    _GBenergyIn(getParam<Real>("GBenergyIn")),
    _GBenergyOut(getParam<Real>("GBenergyOut")),
    _GBMobility(getParam<Real>("GBMobility")),
    _molar_vol(getParam<Real>("molar_volume")),

    _cut_off(getParam<Real>("cut_off")),
    _T(coupledValue("T")),
    _v(coupledValue("v")),
...
void
GBDiscountinousEnergy::computeQpProperties()
{
  Real M0 = _GBmob0;
