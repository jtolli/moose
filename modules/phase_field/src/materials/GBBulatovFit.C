    ...
    _kb(8.617343e-5) //Boltzmann constant in eV/K
{
  ...
  _euler_angles[0](0) = 0;
  _euler_angles[0](1) = 17;
  _euler_angles[0](2) = 31;
  _euler_angles[1](0) = 3;
  _euler_angles[1](1) = 47;
  _euler_angles[1](2) = 93;
...
GBBulatovFit::computeQpProperties()
{
  ...
  std::vector<VariableValue *> vfirst; // values of the order parameter
    RealTensorValue _rotation_matrix;
    RealVectorValue _gbdir;


  _grad_size = std::sqrt((*_grad_v[1])[_qp]*(*_grad_v[1])[_qp]);
  //Moose::out << "grad size " << _grad_size << std::endl;
  _gbdir = (*_grad_v[1])[_qp]/_grad_size; //RealGradient and RealVectorValue are really the same type
  //Moose::out << "normal vector\n" << _gbdir(0) << ' ' << _gbdir(1) << ' ' << _gbdir(2) << std::endl;
...
//  Moose::out << "Rotation Matrix\n" << _rotation_matrix(0,0) << ' '
//    << _rotation_matrix(0,1) << ' ' << _rotation_matrix(0,2) << std::endl
//    << _rotation_matrix(1,0) << ' ' << _rotation_matrix(1,1) << ' ' << _rotation_matrix(1,2) << std::endl
//    << _rotation_matrix(2,0) << ' ' << _rotation_matrix(2,1) << ' ' << _rotation_matrix(2,2) << std::endl;

//    Moose::out << "P\n" << _rotation_tensors[0](0,0) << ' '
//      << _rotation_tensors[0](0,1) << ' ' << _rotation_tensors[0](0,2) << std::endl
//      << _rotation_tensors[0](1,0) << ' ' << _rotation_tensors[0](1,1) << ' ' << _rotation_tensors[0](1,2) << std::endl
//      << _rotation_tensors[0](2,0) << ' ' << _rotation_tensors[0](2,1) << ' ' << _rotation_tensors[0](2,2) << std::endl;


 if (_bnds[_qp] < .99 && _grad_size > 0)
 {
    //Moose::out << "x = " << _q_point[_qp](0) << " & y = " << _q_point[_qp](1) << std::endl;
    // Moose::out << "normal Vector = " << _gbdir << std::endl;
   _GBenergy = gB5DOF(_rotation_matrix*_rotation_tensors[0],
     _rotation_matrix*_rotation_tensors[1]);
//      Moose::out << "Energy = " << _GBenergy << std::endl;
  }
  else
  {
    _GBenergy = getParam<Real>("GBenergy");
...
//  Moose::out << distances.size() << ' ' << ksis.size() << ' ' << etas.size() << ' ' << phis.size() << std::endl;
...
//  Moose::out << "weightedMeanEnergy\n";
 ...
 //Moose::out << "weights (100, 110, 111) = " << sum100 << ' ' << sum110 << ' '
 //   << sum111 << std::endl;
 return _en;
}
