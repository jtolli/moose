...
InputParameters validParams<GBRSWTest>()
{
  ...
  params.addParam<Real>("GBmob0", 9.2124e-9, "Grain boundary mobility prefactor in m^4/(J*s)");
  params.addParam<Real>("Q", 2.77, "Grain boundary migration activation energy in eV");
  ...
  params.addParam<Real>("E_min", 1, "Minimum energy in the grain boundary");
  params.addParam<Real>("E_max", 2, "Maximum energy in the grain boundary");
  params.addRequiredCoupledVar("v", "Order parameter values");
  params.addRequiredCoupledVar("bnds", "grain boundaries");
...
//  if (_grad_size < 0.01)
//     _grad_size = 0.01;
//  if (_mag_grad_comp < 0.01)
//     _mag_grad_comp = 0.01;

  if (_bnds[_qp] < .99)
  {
    if (_mag_grad_comp > std::pow(10,-10))
       _GBenergy = _E_min + (_E_max - _E_min) * _mag_grad_comp/_grad_size * (1 - std::log(_mag_grad_comp/_grad_size));
    else
      _GBenergy = _E_min;
  }
  else
